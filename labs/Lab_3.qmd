...
2.  ***(3 pts)*** Extract the `employees` and `customers` tables from the `company.db` database then convert them to `tibbles`. Use a left join to combine the two tibbles by `AlbumId` in the order `employees` then `customers`.

3.  Repeat number 2 above, but switch the order of the tables to `customers` then `employees`.

4.  ***(3 pts)*** Use a right join to combine the `customers` and `employees` tables by `AlbumId` in the order `employees` then `customers`. What do you notice about this new table? In particular, how does the new table compare to the one in number 3 above?
