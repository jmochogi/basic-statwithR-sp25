...
We can also create density plots to visualize the distribution of the `price` variable by `cooling`. To do this, we use the `geom_density()` function. 
Study the code below before running it. Change the value of the `alpha` argument to 0.9 to see what happens. 
...
4.  **(4 pts)** Create a ***dot plot*** to visualize the distribution of the variable `loan_amount`. Comment on the distribution.

5.  **(4 pts)** Suppose we want to answer the question, "do people with higher `total_income` tend to have higher `loan_amount`?. Create a scatter plot to visualize the relationship between `loan_amount` and `total_income`. Describe this relationship.

6.  **(4 pts)** Create a new ***scatter plot*** that colors the dots on the scatter plot in question 5 above using the categorical variable `has_second_income` (i.e., whether someone has a second income or not). Comment on the new scatter plot.

7.  Create a ***contingency table*** to summarize the counts of the variables `loan_status` and `home_ownership`. Comment on this table. Please look up how to create a contingency table in R using the `tidyverse` workflow.
